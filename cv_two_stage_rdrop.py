# -*- coding: utf-8 -*-
"""
cv_two_stage_rdrop.py
=====================

結合 R-Drop 一致性正則化的兩階段微調實驗。
...
正則化方法：
  - R-Drop：複製批次取得兩組 Dropout 輸出，加入對稱 KL 散度約束。
...
sys.stdout = TeeLogger("two-stage_rdrop_log.txt")
...
class CFG:
    ...
    output_root: str = "./two_stage_rdrop_runs"

    # Excel 報表統一輸出資料夾
    excel_output_dir: str = "./two_stage_rdrop_excel_outputs"
...
def run_phase(
    ...
    if phase_rdrop_alpha > 0.0:
        print(
            f"    -> [R-Drop(alpha={phase_rdrop_alpha})] {phase_name} enabled")
...
def run_training_pipeline(
    ...
    R-Drop 正則化在每個 Phase 皆透過 StrongTrainer 執行。
    """
...
def evaluate_final_test(model_path, tokenizer, test_ds, cfg,
    ...
    if test_texts is not None and test_labels_orig is not None:
        ...
        mis_test_xlsx = os.path.join(
            ...
            f"two_stage_rdrop_test_misclassified_seed{seed}.xlsx",
        )
...
def main(seed: int = 0):
    ...
    test_sample_xlsx = os.path.join(
        cfg.excel_output_dir,
        f"two_stage_rdrop_test_samples_seed{seed}.xlsx",
    )
    ...
    fold_phase_xlsx_name = os.path.join(
        ...
        f"two_stage_rdrop_cv_each_fold_each_phase_metrics_seed{seed}.xlsx",
    ...
    phase_c_xlsx_name = os.path.join(
        ...
        f"two_stage_rdrop_phase_c_each_fold_metrics_seed{seed}.xlsx",
    ...
    mis_xlsx_name = os.path.join(
        cfg.excel_output_dir,
        f"two_stage_rdrop_phase_c_best_misclassified_val_seed{seed}.xlsx",
    )
    ...
    retrain_root = os.path.join(
        cfg.output_root, f"seed_{seed}_retrain_two_stage_rdrop_model")
...
def _save_multi_seed_report(seed_results, suffix="final"):
    ...
    out_name = os.path.join(
        cfg.excel_output_dir,
        f"two_stage_rdrop_multi_seed_report_{suffix}.xlsx",
    )
