...
sys.stdout = TeeLogger("two-stage_log.txt")
...
class CFG:
    ...
    output_root: str = "./two_stage_runs"

    # Excel 報表統一輸出資料夾
    excel_output_dir: str = "./two_stage_excel_outputs"
...
def evaluate_final_test(model_path, tokenizer, test_ds, cfg,
    ...
    if test_texts is not None and test_labels_orig is not None:
        ...
        mis_test_xlsx = os.path.join(
            ...
            f"two_stage_test_misclassified_seed{seed}.xlsx",
        )
...
def main(seed: int = 0):
    ...
    test_sample_xlsx = os.path.join(
        cfg.excel_output_dir,
        f"two_stage_test_samples_seed{seed}.xlsx",
    )
    ...
    fold_phase_xlsx_name = os.path.join(
        cfg.excel_output_dir,
        f"two_stage_cv_each_fold_each_phase_metrics_seed{seed}.xlsx",
    )
    ...
    phase_c_xlsx_name = os.path.join(
        cfg.excel_output_dir,
        f"two_stage_phase_c_each_fold_metrics_seed{seed}.xlsx",
    )
    ...
    mis_xlsx_name = os.path.join(
        cfg.excel_output_dir,
        f"two_stage_phase_c_best_misclassified_val_seed{seed}.xlsx",
    )
    ...
    retrain_root = os.path.join(
        cfg.output_root, f"seed_{seed}_retrain_two_stage_model")
...
def _save_multi_seed_report(seed_results, suffix="final"):
    ...
    out_name = os.path.join(
        cfg.excel_output_dir,
        f"two_stage_multi_seed_report_{suffix}.xlsx",
    )
