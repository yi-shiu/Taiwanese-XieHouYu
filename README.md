...
本專案以 [BERT-base-Chinese](https://huggingface.co/bert-base-chinese) 為骨幹，從 [T-BERT](https://huggingface.co/yixiuuu/tbert-base) 詞表中擷取臺語專有子詞並以其預訓練 Embedding 初始化，擴充模型對臺語詞彙的表徵能力。訓練採兩階段微調架構：Stage 1 凍結 Encoder 僅訓練 Embedding 與分類器；Stage 2 由高層至低層漸進式解凍，搭配層級學習率衰減以保留預訓練知識。並進一步比較 R-Drop 一致性正則化與 FGM 對抗訓練對分類效能及跨種子穩定性的影響。
...
```
.
├── xiehouyu0.csv                # 自建歇後語資料集（2,268 筆，三類情感標籤）
│
...
4. **網路爬蟲** 以 Python Selenium 搭配 BeautifulSoup4 爬取下列公開網頁，共取得候選樣本 1,279 筆，經格式清理、去重複及人工審查後，保留 163 筆正向、265 筆中性樣本納入資料集
...
python analyze_token_lengths.py --data xiehouyu0.csv --tokenizer bert-base-chinese
