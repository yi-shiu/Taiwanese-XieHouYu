# -*- coding: utf-8 -*-
"""
cv_bert_base_chinese.py
==========================

使用 bert-base-chinese 進行情感極性分類之單階段微調實驗。
...
sys.stdout = TeeLogger("bert_base_chinese_log.txt")
...
class CFG:
    ...
    output_root: str = "./bert_base_chinese_runs"

    # Excel 報表統一輸出資料夾
    excel_output_dir: str = "./bert_base_chinese_excel_outputs"
...
def evaluate_final_test(
    ...
    if test_texts is not None:
        ...
        mis_test_xlsx = os.path.join(
            ...
            f"bert_base_chinese_lr{eval_lr_tag}_test_misclassified_seed{seed}.xlsx",
        )
...
def main(seed: int = 0):
    ...
    test_sample_xlsx = os.path.join(
        cfg.excel_output_dir,
        f"bert_base_chinese_lr{lr_tag}_test_samples_seed{seed}.xlsx",
    )
    ...
    cv_xlsx_name = os.path.join(
        cfg.excel_output_dir,
        f"bert_base_chinese_lr{lr_tag}_cv_each_fold_metrics_seed{seed}.xlsx",
    )
    ...
    mis_val_xlsx = os.path.join(
        cfg.excel_output_dir,
        f"bert_base_chinese_lr{lr_tag}_best_misclassified_val_seed{seed}.xlsx",
    )
    ...
    retrain_output_dir = os.path.join(
        cfg.output_root, f"lr_{lr_tag}_seed_{seed}_retrain_bert_base_chinese_model")
...
def _save_multi_lr_report(lr_results, seed, suffix="final"):
    ...
    out_name = os.path.join(
        cfg.excel_output_dir,
        f"bert_base_chinese_multi_lr_seed{seed}_report_{suffix}.xlsx",
    )
