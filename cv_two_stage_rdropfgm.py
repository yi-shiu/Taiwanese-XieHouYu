# -*- coding: utf-8 -*-
"""
cv_two_stage_rdropfgm.py
========================

結合 R-Drop 一致性正則化與 FGM 對抗式訓練的兩階段微調實驗。
...
正則化與對抗式訓練方法：
  - R-Drop：R-Drop：複製批次取得兩組 Dropout 輸出，加入對稱 KL 散度約束。
...
sys.stdout = TeeLogger("two-stage_rdropfgm_log.txt")
...
class CFG:
    ...
    use_rdrop: bool = True
    rdrop_enabled_phases: Tuple[str, ...] = (
    ...
    output_root: str = "./two_stage_rdropfgm_runs"

    # Excel 報表統一輸出資料夾
    excel_output_dir: str = "./two_stage_rdropfgm_excel_outputs"
...
class FGM:
    ...
    透過 StrongTrainer.compute_loss() 結合 R-Drop 正則化。

    每個訓練步驟：
    1. 計算乾淨損失（啟用時包含 R-Drop）並反向傳播，累積乾淨梯度。
    2. 備份乾淨梯度；執行獨立的前向/反向傳遞以取得攻擊方向。
    3. 透過 attack() 擾動 Embedding；還原乾淨梯度；計算對抗損失（啟用時包含 R-Drop）並反向傳播。
    4. 呼叫 restore() 還原 Embedding；優化器使用平均梯度更新參數。
    """
...
class StrongTrainer(Trainer):
    ...
    整合以下功能的自訂 HuggingFace Trainer：
      - 加權交叉熵損失（類別權重、標籤平滑）
      - R-Drop 正則化（複製批次後透過單次前向傳遞取得兩組輸出）
    ...
    def compute_kl_loss(self, p_logits, q_logits):
        """計算兩組輸出分佈之間的對稱 KL 散度：0.5 * (KL(p||q) + KL(q||p))。

        Args:
            p_logits: 第一組未正規化 logits，形狀為 (batch_size, num_labels)。
            q_logits: 第二組未正規化 logits，形狀為 (batch_size, num_labels)。
        """
        p_log = F.log_softmax(p_logits, dim=-1)
        q_log = F.log_softmax(q_logits, dim=-1)
        kl_pq = F.kl_div(p_log, q_log, reduction="batchmean", log_target=True)
        kl_qp = F.kl_div(q_log, p_log, reduction="batchmean", log_target=True)
        return 0.5 * (kl_pq + kl_qp)

    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        """計算訓練損失。

        R-Drop 啟用時：將批次複製後進行單次前向傳遞，取得兩組 logits，
        損失 = 加權交叉熵（兩組平均）+ alpha * 對稱 KL 散度。
        R-Drop 未啟用或評估模式時：退化為標準加權交叉熵損失。
        類別權重與標籤平滑透過 nn.CrossEntropyLoss 套用。
        """
        ...
        if self.rdrop_alpha <= 0.0 or not model.training:
            # R-Drop 未啟用或處於評估模式：標準單次前向傳遞
            outputs = model(**clean_inputs)
            loss = self.loss_fct(
                outputs.logits.view(-1, outputs.logits.size(-1)), flat_labels)
            return (loss, outputs) if return_outputs else loss

        # R-Drop：將批次複製後進行單次前向傳遞，得到兩組 logits
        # 步驟 1：沿批次維度拼接輸入（batch 16 → 32）
        concat_inputs = {k: torch.cat([v, v], dim=0)
                         for k, v in clean_inputs.items()}

        # 步驟 2：一次前向傳遞在不同 Dropout 遮罩下產生兩組輸出
        outputs = model(**concat_inputs)

        # 步驟 3：將 logits 切分為兩半
        logits1, logits2 = torch.chunk(outputs.logits, 2, dim=0)
        num_labels = logits1.size(-1)

        # 步驟 4：對兩半的交叉熵損失取平均
        ce1 = self.loss_fct(logits1.view(-1, num_labels), flat_labels)
        ce2 = self.loss_fct(logits2.view(-1, num_labels), flat_labels)
        ce_loss = 0.5 * (ce1 + ce2)

        # 步驟 5：計算兩組輸出分佈之間的對稱 KL 散度
        kl_loss = self.compute_kl_loss(logits1, logits2)

        # 步驟 6：最終損失 = CE + alpha * KL
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, torch.Tensor], *args, **kwargs):
        ...
        # 步驟 1：乾淨前向 + 反向傳遞（啟用時 compute_loss 包含 R-Drop）
        ...
        # 步驟 3：施加 FGM 擾動；還原乾淨梯度；對抗前向 + 反向傳遞
        # （對抗 compute_loss 啟用時同樣包含 R-Drop）
...
def run_phase(
    ...
    # use_rdrop=True 且 Phase 在 rdrop_enabled_phases 中時啟用 R-Drop；否則 alpha=0（標準 CE）
    rdrop_on = cfg.use_rdrop and (phase_name in cfg.rdrop_enabled_phases)
    phase_rdrop_alpha = cfg.rdrop_alpha if rdrop_on else 0.0

    # use_fgm=True 且 Phase 在 fgm_enabled_phases 中時啟用 FGM
    phase_use_fgm = cfg.use_fgm and (phase_name in cfg.fgm_enabled_phases)

    tag = []
    if phase_rdrop_alpha > 0.0:
        tag.append(f"R-Drop(alpha={phase_rdrop_alpha})")
    if phase_use_fgm:
...
def run_training_pipeline(
    ...
    """
    結合 R-Drop 與 FGM 的兩階段微調 Pipeline：
    ...
    R-Drop 與 FGM 對抗式訓練在每個 Phase 皆透過 StrongTrainer 執行。
    """
...
def evaluate_final_test(model_path, tokenizer, test_ds, cfg,
    ...
    if test_texts is not None and test_labels_orig is not None:
        ...
        mis_test_xlsx = os.path.join(
            ...
            f"two_stage_rdropfgm_test_misclassified_seed{seed}.xlsx",
        )
...
def main(seed: int = 0):
    ...
    test_sample_xlsx = os.path.join(
        cfg.excel_output_dir,
        f"two_stage_rdropfgm_test_samples_seed{seed}.xlsx",
    )
    ...
    fold_phase_xlsx_name = os.path.join(
        ...
        f"two_stage_rdropfgm_cv_each_fold_each_phase_metrics_seed{seed}.xlsx",
    ...
    phase_c_xlsx_name = os.path.join(
        ...
        f"two_stage_rdropfgm_phase_c_each_fold_metrics_seed{seed}.xlsx",
    ...
    mis_xlsx_name = os.path.join(
        cfg.excel_output_dir,
        f"two_stage_rdropfgm_phase_c_best_misclassified_val_seed{seed}.xlsx",
    )
    ...
    retrain_root = os.path.join(
        cfg.output_root, f"seed_{seed}_retrain_two_stage_rdropfgm_model")
...
def _save_multi_seed_report(seed_results, suffix="final"):
    ...
    out_name = os.path.join(
        cfg.excel_output_dir,
        f"two_stage_rdropfgm_multi_seed_report_{suffix}.xlsx",
    )
